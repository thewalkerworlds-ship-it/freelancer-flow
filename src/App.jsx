...
function App() {
    ...
    const exportToExcel = (projects) => {
        ...
        const htmlString = `<html xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta charset="UTF-8"><style>table, th, td { border: 1px solid black; border-collapse: collapse; } .title { background-color: #0B5394; color: white; font-size: 16pt; font-weight: bold; text-align: center; height: 40px; vertical-align: middle; } .header { background-color: #E6F2D4; font-weight: bold; text-decoration: underline; } .total-label { background-color: #FFD966; font-weight: bold; } .total-value { background-color: #FFD966; font-weight: bold; mso-number-format:"\\#\\,##0"; }</style></head><body><table><tr><td class="title" colspan="5">Freelancer Dashboard</td></tr><tr></tr><tr><td class="header">S.No.</td><td class="header">Date</td><td class="header">Project Name</td><td class="header">Client</td><td class="header">Amount</td></tr>${formattedData.map(p => `<tr><td>${p.s_no}</td><td>${p.date}</td><td>${p.projectName}</td><td>${p.client}</td><td style="mso-number-format:'\\#\\,##0'">${p.amount}</td></tr>`).join('')}<tr></tr><tr><td class="total-label" colspan="4">TOTAL EARNINGS</td><td class="total-value">${totalEarnings}</td></tr></table></body></html>`;
    ...
    const handleSubmit = (e) => {
        ...
        if (formData.projectName && formData.clientName && formData.amount && formData.date) {
            const newProject = { id: Date.now().toString(), name: formData.projectName, client: formData.clientName, amount: parseFloat(formData.amount), date: formData.date };
    ...
    const handleSave = () => { if (editFormData) { setProjects(projects.map((p) => (p.id === editingId ? editFormData : p)).sort((a, b) => new Date(b.date) - new Date(a.date))); setEditingId(null); setEditFormData(null); } };
    ...
    function CustomConfirmModal({ onConfirm, onCancel }) { return (<div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex justify-center items-center z-50"><div className="rounded-xl p-1 neon-border w-full max-w-sm" style={{'--card-color': 'var(--destructive)'}}><div className="bg-black/90 rounded-lg"><CardHeader><CardTitle className="text-[var(--destructive)] neon-text text-2xl">Confirm Deletion</CardTitle><CardDescription>Are you certain? This action cannot be undone.</CardDescription></CardHeader><CardContent className="flex justify-end gap-4"><Button variant="outline" onClick={onCancel} className="border-[var(--secondary)]/50 text-[var(--secondary)] hover:bg-[var(--secondary)]/10 neon-border" style={{'--card-color': 'var(--secondary)'}}>Cancel</Button><Button onClick={onConfirm} className="bg-[var(--destructive)] text-white hover:bg-[var(--destructive)]/90 neon-glow" style={{'--glow-color': 'var(--destructive)'}}>Delete</Button></CardContent></div></div></div>); }
    const StatsCard = memo(({ totalEarnings }) => (<Card className="neon-border" style={{'--card-color': 'var(--primary)'}}><CardHeader className="pb-3"><CardDescription>Total Earnings</CardDescription></CardHeader><CardContent><div className="flex items-center gap-4"><div className="flex h-16 w-16 items-center justify-center rounded-xl bg-[var(--primary)]/20 text-[var(--primary)]"><IndianRupee className="h-8 w-8" /></div><div className="space-y-1"><p className="font-mono text-5xl font-bold text-[var(--primary)] neon-text">₹{totalEarnings.toLocaleString("en-IN")}</p><div className="flex items-center gap-2 text-sm text-[var(--accent)]"><TrendingUp className="h-4 w-4" /><span className="font-medium">+12.5% from last month</span></div></div></div></CardContent></Card>));
    const ProjectRow = memo(({ project, index, editingId, editFormData, setEditFormData, handleEdit, handleSave, handleCancel, handleDelete }) => {
        ...
        return (<TableRow className="border-b border-[var(--border)]/30"><TableCell className="text-[var(--muted-foreground)] font-medium border-r border-[var(--border)]/30 text-center">{index + 1}</TableCell><TableCell className="font-semibold text-foreground border-r border-[var(--border)]/30">{isEditing ? <Input type="date" value={editFormData?.date || ""} onChange={(e) => setEditFormData({ ...editFormData, date: e.target.value })} className="h-9 neon-border" style={{'--card-color': 'var(--primary)'}}/> : displayDate}</TableCell><TableCell className="font-semibold text-foreground border-r border-[var(--border)]/30">{isEditing ? <Input value={editFormData?.name || ""} onChange={(e) => setEditFormData({ ...editFormData, name: e.target.value })} className="h-9 neon-border" style={{'--card-color': 'var(--primary)'}}/> : project.name}</TableCell><TableCell className="text-foreground border-r border-[var(--border)]/30">{isEditing ? <Input value={editFormData?.client || ""} onChange={(e) => setEditFormData({ ...editFormData, client: e.target.value })} className="h-9 neon-border" style={{'--card-color': 'var(--secondary)'}}/> : project.client}</TableCell><TableCell className="text-right font-mono font-bold text-foreground border-r border-[var(--border)]/30 text-lg">{isEditing ? <Input type="number" value={editFormData?.amount || ""} onChange={(e) => setEditFormData({ ...editFormData, amount: parseFloat(e.target.value) || 0 })} className="h-9 text-right neon-border" step="0.01" style={{'--card-color': 'var(--accent)'}}/> : `₹${project.amount.toLocaleString("en-IN")}`}</TableCell><TableCell className="text-center">{isEditing ? (<div className="flex items-center justify-center gap-2"><Button size="sm" onClick={handleSave} className="h-9 px-3 bg-[var(--accent)]/20 text-[var(--accent)] hover:bg-[var(--accent)]/30 border border-[var(--accent)] neon-border" style={{'--card-color': 'var(--accent)'}}><Save className="h-4 w-4" /></Button><Button size="sm" variant="outline" onClick={handleCancel} className="h-9 px-3 border border-[var(--secondary)] text-[var(--secondary)] hover:bg-[var(--secondary)]/10 neon-border" style={{'--card-color': 'var(--secondary)'}}><X className="h-4 w-4" /></Button></div>) : (<div className="flex items-center justify-center gap-2"><Button size="sm" variant="outline" onClick={() => handleEdit(project)} className="h-9 px-3 border border-[var(--primary)] text-[var(--primary)] hover:bg-[var(--primary)]/10 neon-border" style={{'--card-color': 'var(--primary)'}}><Edit className="h-4 w-4" /></Button><Button size="sm" variant="outline" onClick={() => handleDelete(project.id)} className="h-9 px-3 border border-[var(--destructive)] text-[var(--destructive)] hover:bg-[var(--destructive)]/10 neon-border" style={{'--card-color': 'var(--destructive)'}}><Trash2 className="h-4 w-4" /></Button></div>)}</TableCell></TableRow>);
    });
